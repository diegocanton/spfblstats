...
'stats')
	...
	criaLogTemp(){
		zegrep " SPFTCP[0-9]+ SPFBL " $LOGFILE > $LOGTEMP
		zegrep " DNSUDP[0-9]+ DNSBL " $LOGFILE > $LOGTEMPDNS
	}
	...
	executaStatsDNSBL(){
		...
		DNSBLBLOCK=$(zegrep -c "A .* => [0-9]+ 127.0.0.2" "$LOGFILE")
		DNSBLSPAM=$(zegrep -c "A .* => [0-9]+ 127.0.0.3" "$LOGFILE")
		DNSBLOK=$(zegrep -c "A .* => [0-9]+ NXDOMAIN" "$LOGFILE")
